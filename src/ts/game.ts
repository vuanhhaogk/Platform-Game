let gameState = {
    ...
    create: function(){
        ...
        let margin_left: number = (game.world.width - game.const.BSIZE * width)/2
        let margin_top: number = (game.world.height - game.const.BSIZE * height)/2

        let level_text = game.add.text(10, 10, `Level: ${game.global.level + 1}/${game.cache.getJSON('levels').length}`, { font: '12px monospace', fill: '#ffffff' })
        ...
        for (let r: number = 0; r < height; r++)
            for (let c: number = 0; c < width; c++){
                let x: number = c * game.const.BSIZE + margin_left
                let y: number = r * game.const.BSIZE + margin_top
    ...
    update: function(){
        ...
        game.physics.arcade.overlap(this.player, this.traps, this.gameover, null, this)
    ...
    collectCoin: function(player, coin){
        ...
        coin_tween.onComplete.addOnce(() => coin.kill())
        coin_tween.start()
        if (coin.alive){
            this.target --
            this.target_text.text = 'Target: ' + this.target
        ...
        if (this.target === 0 && game.cache.getJSON('levels')[game.global.level + 1]){
            game.global.level ++
            game.state.start('game')
        }
    }
